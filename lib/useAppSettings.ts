...
const STORAGE_KEY = "sportsTracker_settings_v1";
export const DEFAULT_SPORT_ORDER: League[] = ["mlb", "nba", "nhl", "nfl", "cfb", "cbb"];
const DEFAULT_SETTINGS: AppSettings = {
  density: "expanded",
  sportOrder: DEFAULT_SPORT_ORDER,
};
...
function normalize(raw: any): AppSettings {
  ...
  return { density, sportOrder: [...cleaned, ...missing] };
}
...
function setStore(next: AppSettings) {
  currentSettings = normalize(next);
...
export function useAppSettings() {
  ...
  const setDensity = useCallback((density: ScoreDensity) => {
    setStore({ ...settings, density });
  }, [settings]);
  ...
  return { settings, setDensity, moveSport, resetSettings };
}
