        ...
        <table className="w-full text-[10px] sm:text-xs table-fixed">
          <thead>
            <tr style={{ background: "var(--surface-2)", color: "var(--text-3)" }}>
              <th className="text-left px-1.5 py-2 font-black">Team</th>
              {Array.from({ length: innings }).map((_, i) => <th key={i} className="text-center px-1 py-2 font-black">{i + 1}</th>)}
              <th className="text-center px-1 py-2 font-black tracking-wide">R</th>
              <th className="text-center px-1 py-2 font-black tracking-wide">H</th>
              <th className="text-center px-1 py-2 font-black tracking-wide">E</th>
            </tr>
          </thead>
          <tbody>
            {teams.map((t: any) => (
              <tr key={t.id || t.abbr} style={{ borderTop: "1px solid var(--border)" }}>
                <td className="px-1.5 py-2 font-bold">
                  <div className="flex items-center gap-1.5">
                    {t.logo && <Image src={t.logo} alt={t.abbr || "Team"} width={18} height={18} className="relative top-[1px] object-contain shrink-0 logo-outline-dark" unoptimized />}
                    <span className="relative top-[2px] leading-none">{t.abbr}</span>
                  </div>
                </td>
